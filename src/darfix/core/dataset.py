...
class ImageDataset:
    ...
    def in_memory(self):
        warnings.warn(
            "in_memory attribute has been removed. Data always in memory",
            DeprecationWarning,
            stacklevel=2,
        )
        return True
    ...
    def get_data(self, indices=None, dimension=None, return_indices=False):
        ...
        :param array_like dimension: If not None, return only the data corresponding to
            ...
            The call get_data(dimension=[[1, 2], [2, 3]]) is equivalent to data[:, 2, 3].
    ...
    def _waterfall_nmf(
        ...
        for i in range(len(iterations)):
            ...
            data = Data(new_urls, self.get_data(indices).metadata)
    ...
    def apply_moments(
        ...
        for axis, dim in self.dims.items():
            ...
            self.moments_dims[axis] = numpy.asarray(
    ...
    def apply_fit(
        ...
        with self.state_of_operations.run_context(Operation.FIT):
            ...
            for i, image in enumerate(fitted_data):
                j = indices[i] if indices is not None else i
                filename = os.path.join(_dir, "data_fit" + str(j).zfill(4) + ".npy")
